...
class PoseMatchGame:
    ...
    def _update_countdown(self) -> None:
        ...
        if left > 0:
            self._draw_center(str(left), self.fonts["title"], CAMERA_HEIGHT // 2 - 40, (0,255,0))
            self._draw_center("GET READY!",self.fonts["large"], CAMERA_HEIGHT // 2 + 40, (0,255,0))
        else:
    ...
    def _update_gameplay(self) -> None:
        ...
        is_celebrating = time.time() < self.overlay_end_time
        ...
        if not is_celebrating:
            ...
            if matched:
                pose_name = self.session.current_pose.name
                
                if pose_name == "PLUS SIA NOO":
                    self.overlay_text = "RUUAY MAI WAI LAEW"
                elif pose_name == "ABSALUTE CINEMA":
                    self.overlay_text = "absalute\n CINEMA"
                else:
                    self.overlay_text = "== YEAH =="
                
                # ค้างหน้าจอ 2 วินาที
                self.overlay_end_time = time.time() + 2.0
                self.session.complete_pose()
                self.session.start_time += 2.0 
        else:
            keypoints = {}
            matched = False
        ...
        self.last_frame_surface = self._frame_to_surface(frame)
        self._draw_gameplay(keypoints_detected=bool(keypoints), matched=matched, is_celebrating=is_celebrating)
    ...
    def _draw_gameplay(self, keypoints_detected: bool, matched: bool, is_celebrating: bool) -> None:
        self.screen.fill((8, 10, 14))
        if self.last_frame_surface:
            self.screen.blit(self.last_frame_surface, (0, 0))
        ...
        if is_celebrating:
            ...
            y0 = CAMERA_HEIGHT // 2 - (len(lines) * 35) + 35
    ...
    def _preview_points(self, pose_id: int) -> dict[str, tuple[float, float]]:
        ...
        presets = {
            ...
            22: {"left_elbow": (0.45, 0.45), "left_wrist": (0.60, 0.35), "right_elbow": (0.55, 0.45), "right_wrist": (0.40, 0.35)},
